...
This Wi-Fi connection is made possible from two Ubiquiti bullet directional radios pointed towards Coihueco.
...
A GPS antenna, located on the roof of the CRS, connects to a GPS clock and level shifter system to provide the PPS and \SI{10}{\mega\hertz} input signals to the WR switch.
...
To this end, the WR switch connects via ethernet cables to the FZ-55 toughbook in two locations: 1. from the WR switch management port, and 2. from WR switch SFP port 17, converted for ethernet cable connections with an SFP to RJ45 transceiver module.
...
\begin{figure}
    ...
    \caption[Picture of a White Rabbit switch.]{A picture of the White Rabbit (WR) switch deployed inside the CRS. The RJ45 port labeled "MANAGEMENT" is the WR management port. Directly next to the management port are two SMC male connectors to take the PPS and \SI{10}{\mega\hertz} in signals from the GPS clock defining the master time signal of the WR system. The 18 SFP ports are used for providing sub-nanosecond synchronization to any connected components. For the setup inside the CRS, port 17 is used as a communications channel to the FZ-55 toughbook.}
    \label{fig:wr_switch}
\end{figure}

Connection to WR switch port 17 forms the communications channel between the WR switch and FZ-55 toughbook.
...
This information is all sent bidirectionally through the buried optical fiber to the WR switch, and can be accessed from the FZ-55 toughbook via WR switch port 17.
...
The \SI{12}{\volt} DC output is used to power the WR switch, while the \SI{24}{\volt} output line can be used to power a station DAQ electronics board either in the field or inside the CRS for debugging.
The specific ports used to power the laptop and WR switch can be controlled from an IP network switch, allowing remote power cycling of each component via the network GUI, with a physical switch in the case of the custom DC-DC converter used for the WR switch.
~\Cref{fig:skala_control} shows a diagram of all connections for power, networking, communications, and data transfer inside the CRS, including the connection to individual SKALA stations from the WR switch.
...
The \SI{24}{\volt} power is split into three separate DC-DC power converters of voltages \SI{3.3}{\volt}, \SI{4.5}{\volt}, and \SI{5}{\volt} to provide power to all TAXI components and all additional electronics inside the TAXI housing.% connected to the TAXI-v3.2 board. 
...
These timing signals originate at the WR switch in the CRS from the GPS locked master timing signal.
