...
A parameterized Gaisser-Hillas function, described in~\Cref{sec:development}, was used to fit the CORSIKA simulated electron-positron longitudinal profile.
Extracted from the fit are the depth of shower maximum ($X_{\rm max}$), the shower width ($L$), the shower asymmetry ($R$), and the electron-positron number at shower maximum ($N_{\rm e, max}$).
$N_{\rm e, max}$ can be reconstructed by either fluorescence telescopes or radio detectors and will be used in the context of this analysis as a proxy to the true air-shower energy for scaling the other observables. 
...
Refer to~\Cref{chap:airshowers} for descriptions of the detection techniques used for the other studied mass-sensitive observables. 
...
This choice is discussed in~\Cref{sec:annulus}. 
